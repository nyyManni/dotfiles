...
#+begin_src emacs-lisp :tangle yes
  ...
  (defun my-config-evil ()
    ...
    (define-key evil-normal-state-map (kbd "ö")       'helm-find-files)
    (define-key evil-normal-state-map (kbd "ä")       'projectile-switch-project)
...
#+begin_src emacs-lisp :tangle yes
  ...
  ;; When exporting asynchronously, use the export config.
  (setq org-export-async-init-file (concat user-emacs-directory "/org-async-init.el"))

  (define-key org-mode-map (kbd "S-<f3>") 'my-org-pdf-async)
